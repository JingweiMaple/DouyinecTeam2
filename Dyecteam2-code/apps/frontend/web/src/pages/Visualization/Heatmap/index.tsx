...
// ⭐ 使用封装好的接口
import { fetchRegionHeatmap } from '../../../services/logistics';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 使用 axios 封装的接口；拦截器已经返回 { code, message, data }
        const res = await fetchRegionHeatmap();
        const { code, message, data } = res as {
          code?: number;
          message?: string;
          data?: { points?: OrderHeatPoint[] };
        };

        if (code === 0 && data) {
          const points = (data.points || []) as OrderHeatPoint[];
          setAllPoints(points);
        } else {
          MessagePlugin.error(message || '获取区域订单热力图数据失败');
        }
      } catch (err) {
        console.error(err);
